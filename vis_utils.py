...
def format_attention(attention, layers=None, heads=None):
    ...
    for layer_attention in attention:
        ...
        layer_attention = tf.squeeze(layer_attention, 0)
        if heads:
            layer_attention = layer_attention[heads]
        squeezed.append(layer_attention)
    # num_layers x num_heads x seq_len x seq_len
    return tf.stack(squeezed)
...
def plot_decoder_text_attention(
    ...
    if attention is not None:
        ...
        if sentence_b_start is None:
            ...
            attention = tf.reshape(attention, (1, 1, attention.shape[-1], attention.shape[-1]))
            attn_data.append(
                {
                    'name': None,
                    'attn': attention.numpy().tolist(),
                    'left_text': tokens,
                    'right_text': tokens
                }
            )
